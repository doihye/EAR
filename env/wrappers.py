...
def make_pad_env(
	...
	env = ColorWrapper(env, mode)	# -> .reset first
...
class ColorWrapper(gym.Wrapper):
	...
	def reset(self):
		...
		return self.env.reset() # --> framestack.reset
...
class FrameStack(gym.Wrapper):
	...
	def reset(self):
		...
		return self._get_obs()	# --> greenscreen.reset()
...
class GreenScreen(gym.Wrapper):
	...
	def __init__(self, env, mode):
		...
		if 'video' in mode:
			...
			if not self._video.endswith('.mp4'):
				self._video += '.mp4'
			self._video = os.path.join('./env/data', self._video)
	...
	def reset(self):
		...
		return self._greenscreen(self.env.reset()) # --> dmc2gym reset
